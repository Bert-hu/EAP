...
namespace HandlerAgv.Service.Services
{
    public class QuartzUtil
    {
        ...
        /// <returns></returns>
        public static async Task AddJob<T>(string JobName, string CronTime, Dictionary<string, object> jobDataMap) where T : IJob
        {
        ...
        /// <param name="jobDataMap"></param>
        /// <returns></returns>
        public static async Task AddJob<T>(string JobName, DateTimeOffset StartTime, DateTimeOffset EndTime, int SimpleTime, Dictionary<string, object> jobDataMap) where T : IJob
        {
            await AddJob<T>(JobName, StartTime, EndTime, TimeSpan.FromMilliseconds(SimpleTime), jobDataMap);
        }
        ...
        /// <param name="jobDataMap"></param>
        /// <returns></returns>
        public static async Task AddJob<T>(string JobName, DateTimeOffset StartTime, DateTimeOffset EndTime, TimeSpan SimpleTime, Dictionary<string, object> jobDataMap) where T : IJob
        {
            IJobDetail jobCheck = JobBuilder.Create<T>().WithIdentity(JobName, JobName + "_Group").Build();
            jobCheck.JobDataMap.PutAll(jobDataMap);
        ...
        /// <param name="SimpleTime"></param>
        /// <returns></returns>
        public static async Task AddJob<T>(string JobName, TimeSpan SimpleTime) where T : IJob
        {
            IJobDetail jobCheck = JobBuilder.Create<T>().WithIdentity(JobName, JobName + "_Group").Build();
            ITrigger trigger = TriggerBuilder.Create().WithIdentity(JobName + "_SimpleTrigger", JobName + "_TriggerGroup")
                .StartNow()
                 .WithSimpleSchedule(t => t.WithInterval(SimpleTime).RepeatForever())
                .Build();
            await sched.ScheduleJob(jobCheck, trigger);
        }
